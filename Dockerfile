...
RUN apk upgrade --no-cache  && docker-php-ext-install pcntl && rm -rf /usr/src/* && rm -rf /var/cache/apk/*
...
RUN mkdir /opt/hapcoll/conf /opt/hapcoll/include
COPY conf/* /opt/hapcoll/conf/
COPY include/* /opt/hapcoll/include/
